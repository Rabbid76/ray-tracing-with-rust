...
impl RotateY {
    pub fn new(angle_radians: FSize, node: Arc<dyn Geometry>) -> RotateY {
        RotateY::new_id(Object::new_id(), angle_radians, node)
    }

    pub fn new_id(id: usize, angle_radians: FSize, node: Arc<dyn Geometry>) -> RotateY {
        let mut r = RotateY {
            id,
...
impl Geometry for RotateY {
    ...
    fn accept(&self, visitor: &mut dyn Visitor) -> Result<(), Box<dyn Error>> {
        visitor.visit_instancing_rotate_y(&self)
    }
}
...
mod rotate_y_test {
    ...
    fn bounding_box_test() {
        ...
        let i = RotateY::new(FSize::to_radians(30.0), Arc::new(c));
        let b = i.bounding_box(0.0..0.0);
        match b {
            Some(b) => {
                test::assert_eq_vector3(&b.min, &Vector3::new(-1.366, -1.0, -1.366), 0.01);
                test::assert_eq_vector3(&b.max, &Vector3::new(1.366, 1.0, 1.366), 0.01);
            }
            _ => assert!(false),
        }
    }
