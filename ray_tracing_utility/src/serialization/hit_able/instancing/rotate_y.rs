...
impl RotateY {
    pub fn from_hit_able(i: &instancing::RotateY) -> Result<RotateY, Box<dyn Error>> {
        Ok(RotateY {
    ...
    ) -> Result<instancing::RotateY, Box<dyn Error>> {
        Ok(instancing::RotateY::new_id(
...
mod rotate_y_test {
    ...
    #[test]
    fn rotate_y_test_from_rotate_y() {
        ...
        let s_id = s.id;
        let i = instancing::RotateY::new(FSize::to_radians(30.0), Arc::new(s));
        let n = RotateY::from_hit_able(&i).unwrap();
        test::assert_eq_float(n.angle.to_value().unwrap(), 30.0, 0.001);
        assert_eq!(n.node, IdReference::Single(s_id));
    }

    #[test]
    fn rotate_y_test_to_rotate_y() {
        let f = RotateY {
